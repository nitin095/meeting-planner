...
export class ProfileComponent implements OnInit {
  ...
  public saveUserDetails() {
    if (this.userType === 'users')
      this.appService.editUser(this.userType, this.userDetails.userId, this.userDetails).subscribe(
        response => {
          if (response.status == 200) {
            this.appService.setUserInfoInLocalStorage(this.userDetails);
            this.snackBar.open('Profile updated sucessfully', 'Close', { verticalPosition: 'top', horizontalPosition: 'end', duration: 4000, });
        ...
        error => {
          this.snackBar.open('Some error occured. Please try again', 'Close', { verticalPosition: 'top', horizontalPosition: 'end', duration: 4000, });
          console.log(error)
        }
      )
    else
      this.appService.editUser(this.userType, this.userDetails.adminId, this.userDetails).subscribe(
        response => {
          if (response.status == 200) {
            this.appService.setUserInfoInLocalStorage(this.userDetails);
            this.snackBar.open('Profile updated sucessfully', 'Close', { verticalPosition: 'top', horizontalPosition: 'end', duration: 4000, });
  ...
  public recoverPassword() {
    this.appService.recoverPassword(this.userType, this.userDetails.email).subscribe(
      response => {
        if (response.status == 200) {
          this.snackBar.open('Password reset email sent', 'Close', { verticalPosition: 'top', horizontalPosition: 'end', duration: 4000, });
        }
      },
      error => {
        console.log(error)
